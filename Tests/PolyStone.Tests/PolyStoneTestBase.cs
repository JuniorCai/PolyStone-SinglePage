...
namespace PolyStone.Tests
{
    public abstract class PolyStoneTestBase : AbpIntegratedTestBase<PolyStoneTestModule>
    {
        ...
        protected void UsingDbContext(Action<PolyStoneDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PolyStoneDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PolyStoneDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PolyStoneDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PolyStoneDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PolyStoneDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PolyStoneDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PolyStoneDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PolyStoneDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PolyStoneDbContext, Task<T>> func)
        {
            T result;
